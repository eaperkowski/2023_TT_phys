...
temp_data <- read.csv("../data/allegheny_co_airport_weather.csv") %>%
  mutate(temp_date = lubridate::ymd(temp_date))
...
tgrow_pre <- tgrow_pre_prep %>%
  merge(temp_data) %>%
  group_by(id, date) %>%
  filter(temp_date >= date - 30 & temp_date < date) %>%
  summarize(tgrow = mean(daily_temp_mean)) %>%
  ungroup(date) %>%
  select(id, tgrow)
...
aci.coefs.pre <- aci.coefs %>%
  ...
  select(id:timepoint, vcmax = Vcmax, jmax = Jmax, rd = Rd, tpu = TPU)
...
tgrow_post_prep[95,] <- list("flag2_tri", lubridate::date("2023-06-12"))
tgrow_post_prep[96,] <- list("flag3_tri", lubridate::date("2023-06-12"))
...
multispeq <- read.csv("../data/raw_data/TT23_multispeq_data.csv") %>%
  select(id:canopy, Phi2, FvP_over_FmP, SPAD, -composite)
