...
vcmax.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
  ...
  scale_y_continuous(limits = c(0, 200)) +
  labs(x = "Total subplot garlic mustard density", 
       y = expression(bold("V"["cmax25"]*" ("*mu*"mol"*" m"^"-2"*" s"^"-1"*")")),
       fill = "Species", color = "Species") +
  theme_bw(base_size = 18) +
...
jmax <- lmer(log(jmax25) ~ gm.trt * spp * canopy + (1 | plot), 
             data = subset(df, spp != "Ari"))
...
cld(emtrends(jmax, pairwise~total_subplot*spp*canopy))

# Create compact letters for plot
jmax.letters <- cld(emmeans(jmax, pairwise~canopy*spp), Letters = LETTERS) %>%
  mutate(.group = trimws(.group, which = "both")) %>% data.frame()

## Create plot
jmax.cat <- ggplot(data = subset(df, spp != "Ari"), 
                    aes(x = canopy, y = jmax25, fill = spp)) +
  geom_boxplot(alpha = 0.75) +
  geom_point(shape = 21, size = 2, alpha = 0.75, 
             position = position_jitterdodge(jitter.width = 0.1, 
                                             dodge.width = 0.75)) +
...
##############################################################################
## Jmax regressed against subplot GM density and canopy openness
##############################################################################
df$jmax25[c(20, 178)] <- NA

jmax.cont <- lmer(log(jmax25) ~ total_subplot * spp * canopy + (1 | plot), 
             data = subset(df, spp != "Ari"))

# Check model assumptions
plot(jmax.cont)
qqnorm(residuals(jmax.cont))
qqline(residuals(jmax.cont))
densityPlot(residuals(jmax.cont))
shapiro.test(residuals(jmax.cont))
outlierTest(jmax.cont)

# Model output
summary(jmax.cont)
Anova(jmax.cont)
r.squaredGLMM(jmax.cont)

# Post-hoc comparisons
test(emtrends(jmax.cont, ~canopy*spp, "total_subplot"))

# Create trendlines for plot
jmax.trend <- emmeans(jmax.cont, ~total_subplot*spp*canopy, 
                       at = list(total_subplot = seq(0, 145, 1)),
                       type = "response") %>% data.frame() %>%
  mutate(linetype = ifelse(spp == "Mai" & canopy == "pre_closure",
                           "solid", "dashed"))
...
## Create plot
jmax.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
                          aes(x = total_subplot, y = jmax25, fill = spp)) +
  geom_point(size = 3, alpha = 0.75, shape = 21) +
  geom_ribbon(data = jmax.trend,
              aes(y = response, ymax = upper.CL, ymin = lower.CL,
                  fill = spp), alpha = 0.3) +
  geom_smooth(data = jmax.trend, method = 'lm', se = FALSE,
              aes(y = response, linetype = linetype, color = spp)) +
  facet_wrap(.~canopy, labeller = as_labeller(facet.labels)) +
  scale_linetype_manual(values = c("dashed", "solid")) +
  scale_y_continuous(limits = c(0, 300), breaks = seq(0, 300, 75)) +
...
##############################################################################
png("../figs/TT23_canopy_photo.png", width = 8, height = 3.5, units = "in",
    res = 600)
ggarrange(vcmax.cat, jmax.cat, ncol = 2, common.legend = TRUE, legend = "right")
dev.off()

png("../figs/TT23_canopy_photo_cont.png", width = 8, height = 8, units = "in",
    res = 600)
ggarrange(vcmax.cont.plot, jmax.cont.plot, nrow = 2, common.legend = TRUE, legend = "right")
dev.off()

##############################################################################
## Jmax:Vcmax
##############################################################################
df$jmax.vcmax[c(179, 225)] <- NA

jvmax <- lmer(jmax.vcmax ~ gm.trt * canopy * spp + (1 | plot), 
              data = subset(df, spp != "Ari"))
...
# Post-hoc comparisons
cld(emmeans(jvmax, pairwise~canopy*spp, type = "response"))
emmeans(jvmax, pairwise~gm.trt, type = "response")

# Create compact letters for plot
jvmax.letters <- cld(emmeans(jvmax, pairwise~canopy*spp), Letters = LETTERS) %>%
  mutate(.group = trimws(.group, which = "both")) %>% data.frame()

## Create plot
jvmax.cat <- ggplot(data = subset(df, spp != "Ari"), 
                   aes(x = canopy, y = jmax.vcmax, fill = spp)) +
  geom_boxplot(alpha = 0.75) +
  geom_point(shape = 21, size = 2, alpha = 0.75, 
             position = position_jitterdodge(jitter.width = 0.1, 
                                             dodge.width = 0.75)) +
  geom_text(data = jvmax.letters, aes(y = 0.8, label = .group),
            position = position_dodge(width = 0.75), size = 5,
            fontface = "bold") +
  scale_x_discrete(labels = c("Open", "Closed")) +
...
##############################################################################
## Jmax:Vcmax regressed along total subplot garlic mustard density
##############################################################################
df$jmax.vcmax[c(179, 225)] <- NA

jvmax.cont <- lmer(jmax.vcmax ~ total_subplot * canopy * spp + (1 | plot), 
              data = subset(df, spp != "Ari"))

# Check model assumptions
plot(jvmax.cont)
qqnorm(residuals(jvmax.cont))
qqline(residuals(jvmax.cont))
densityPlot(residuals(jvmax.cont))
shapiro.test(residuals(jvmax.cont))
outlierTest(jvmax.cont)

# Model output
summary(jvmax.cont)
Anova(jvmax.cont)
r.squaredGLMM(jvmax.cont)

# Post-hoc comparisons
test(emtrends(jvmax.cont, ~spp*canopy, "total_subplot"))

cld(emmeans(jvmax, pairwise~canopy*spp, type = "response"))
emmeans(jvmax, pairwise~gm.trt, type = "response")

# Create trendlines for plot
jvmax.trend <- emmeans(jvmax.cont, ~total_subplot*spp*canopy, 
                      at = list(total_subplot = seq(0, 145, 1)),
                      type = "response") %>% data.frame() %>%
  mutate(linetype = ifelse(spp == "Mai" & canopy == "pre_closure",
                           "solid", "dashed"))
...
## Create plot
jvmax.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
                         aes(x = total_subplot, y = jmax.vcmax, fill = spp)) +
  geom_point(size = 3, alpha = 0.75, shape = 21) +
  facet_wrap(.~canopy, labeller = as_labeller(facet.labels)) +
  scale_y_continuous(limits = c(0.4, 0.8), breaks = seq(0.4, 0.8, 0.1)) +
  labs(x = "Total subplot garlic mustard density", 
...
# Create compact letters for plot
cica.letters <- cld(emmeans(ci.ca, pairwise~canopy*spp), Letters = LETTERS) %>%
  mutate(.group = trimws(.group, which = "both")) %>% data.frame()

## Create plot
cica.cat <- ggplot(data = subset(df, spp != "Ari"), 
                    aes(x = canopy, y = ci.ca, fill = spp)) +
  geom_boxplot(alpha = 0.75) +
  geom_point(shape = 21, size = 2, alpha = 0.75, 
             position = position_jitterdodge(jitter.width = 0.1, 
                                             dodge.width = 0.75)) +
  geom_text(data = jvmax.letters, aes(y = 1, label = .group),
            position = position_dodge(width = 0.75), size = 5,
            fontface = "bold") +
  scale_x_discrete(labels = c("Open", "Closed")) +
...
##############################################################################
## Ci:Ca regressed against subplot GM density and canopy openness
##############################################################################
df$ci.ca[c(160)] <- NA

cica.cont <- lmer(ci.ca ~ total_subplot * spp * canopy + (1 | plot), 
                  data = subset(df, spp != "Ari"))

# Check model assumptions
plot(cica.cont)
qqnorm(residuals(cica.cont))
qqline(residuals(cica.cont))
densityPlot(residuals(cica.cont))
shapiro.test(residuals(cica.cont))
outlierTest(cica.cont)

# Model output
summary(cica.cont)
Anova(cica.cont)
r.squaredGLMM(cica.cont)

# Post-hoc comparisons
test(emtrends(cica.cont, ~canopy*spp, "total_subplot"))

## Create plot
cica.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
                         aes(x = total_subplot, y = ci.ca, fill = spp)) +
  geom_point(size = 3, alpha = 0.75, shape = 21) +
  facet_wrap(.~canopy, labeller = as_labeller(facet.labels)) +
  scale_y_continuous(limits = c(0.4, 1), breaks = seq(0.4, 1, 0.2)) +
  labs(x = "Total subplot garlic mustard density", 
...
gsw <- lmer(gsw ~ gm.trt * spp + (1 | plot), data = subset(df, spp != "Ari"))
...
# Post-hoc comparisons
cld(emmeans(gsw, pairwise~spp*gm.trt), Letters = LETTERS)
...
## Create plot
gsw.cat <- ggplot(data = subset(df, spp != "Ari"), 
...
##############################################################################
## Gsw regressed against subplot GM density and canopy openness
##############################################################################
df$gsw[c(83)] <- NA

gsw.cont <- lmer(sqrt(gsw) ~ total_subplot * spp * canopy + (1 | plot), 
                  data = subset(df, spp != "Ari"))

# Check model assumptions
plot(gsw.cont)
qqnorm(residuals(gsw.cont))
qqline(residuals(gsw.cont))
densityPlot(residuals(gsw.cont))
shapiro.test(residuals(gsw.cont))
outlierTest(gsw.cont)

# Model output
summary(gsw.cont)
Anova(gsw.cont)
r.squaredGLMM(gsw.cont)

# Post-hoc comparisons
test(emtrends(gsw.cont, ~canopy*spp, "total_subplot"))



## Create plot
gsw.cont.plot <- ggplot(data = subset(df, spp != "Ari"), 
                         aes(x = total_subplot, y = gsw, fill = spp)) +
  geom_point(size = 3, alpha = 0.75, shape = 21) +
  facet_wrap(.~canopy, labeller = as_labeller(facet.labels)) +
  scale_y_continuous(limits = c(0.4, 1), breaks = seq(0.4, 1, 0.2)) +
  labs(x = "Total subplot garlic mustard density", 
